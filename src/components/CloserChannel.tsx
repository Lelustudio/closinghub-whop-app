...
function CloserChannelContent() {
	...
	const fetchCards = async (page = 1, limit = 12) => {
		try {
			...
			const response = await fetch(`/api/cards?channel=closer&card_type=closer_profile&limit=${limit}&offset=${offset}`);
        ...
        <main className="min-h-screen p-6">
			...
			<div className="max-w-6xl mx-auto">
				...
				<div className="flex items-center justify-between mb-8">
                           <div>
                               <h1 className="text-4xl font-extrabold text-white mb-2">Closers Channel</h1>
                               <p className="text-white/70">Discover the best available Closer profiles</p>
						<div className="flex items-center gap-4 mt-2">
							{created && (
								<div className="px-4 py-2 bg-green-500/20 border border-green-500/30 rounded-lg">
                                   <p className="text-green-400 text-sm">✅ Profile created successfully!</p>
								</div>
							)}
							...
							{totalCards > 0 && (
								<div className="px-3 py-1 bg-gray-500/20 border border-gray-500/30 rounded-lg">
                                   <p className="text-gray-400 text-xs">
                                       📊 {totalCards} profile{totalCards > 1 ? 's' : ''} total
                                   </p>
								</div>
							)}
						</div>
				...
				{loading && (
                       <div className="text-center py-12">
                           <p className="text-white/50">Loading profiles...</p>
                       </div>
				)}
							...
							>
								<div className="flex items-center gap-4 mb-4">
									...
									<div>
										<h3 className="font-semibold">{card.meta.display_name}</h3>
                                           <p className="text-sm text-white/60">Experience: {card.meta.experience}</p>
									</div>
								</div>
								...
								<button 
									className="w-full py-2 rounded-lg bg-gradient-to-r from-lime-400 to-green-600 font-semibold text-black"
                                           onClick={() => alert('Hire now feature under development')}
								>
									Hire now
								</button>
							</div>
				...
				{!loading && cards.length === 0 && (
                       <div className="text-center mt-12">
                           <p className="text-white/50 text-lg">
                               No Closer profiles have been created yet.
                           </p>
...
export default function CloserChannel() {
	return (
